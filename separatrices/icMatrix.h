...
public:
  ...
  inline icMatrix2x2(double M[2][2]);
	inline icMatrix2x2(const icVector2 &v1, const icVector2 &v2);
  ...
  inline icMatrix2x2 &set			(double M00, double M01, 
					      							 double M10, double M11);
	inline icMatrix2x2 &set     (const icVector2 &v1, const icVector2 &v2);
...
inline icMatrix2x2::icMatrix2x2(const icVector2 &v1, const icVector2 &v2) {
...
inline icMatrix2x2 &icMatrix2x2::set(double M00, double M01, 
																double M10, double M11) {
  entry[0][0] = M00;
  entry[0][1] = M01;
  entry[1][0] = M10;
...
inline icMatrix2x2 &icMatrix2x2::set(const icVector2 &v1, const icVector2 &v2) {
	entry[0][0] = v1.entry[0];
	entry[0][1] = v1.entry[1];
	entry[1][0] = v2.entry[0];
	entry[1][1] = v2.entry[1];
...
inline icMatrix2x2 conjugate(const icMatrix2x2 &a, const icMatrix2x2 &b) {
  icMatrix2x2 tmp(a);
	icMatrix2x2 c = inverse(b);
  tmp.rightMultiply(b);
	tmp.leftMultiply(c);
  return tmp;
}

inline icMatrix2x2 othoconjugate(const icMatrix2x2 &a, const icMatrix2x2 &b) {
  icMatrix2x2 tmp(a);
	icMatrix2x2 c = transpose(b);
  tmp.rightMultiply(b);
	tmp.leftMultiply(c);
  return tmp;
}
...
class icMatrix2x2List {
public:

  int num_matrices;
  int max_matrices;
  icMatrix2x2 **matrices;

  icMatrix2x2List (int max) {
    max_matrices = max;
    matrices = new icMatrix2x2 *[max_matrices];
    num_matrices = 0;
  }
	void append(icMatrix2x2 *m)
	{
		int i;

		if (num_matrices >= max_matrices) {
			max_matrices += 200;
			icMatrix2x2 **mlist = new icMatrix2x2 *[max_matrices];
...
class icMatrix3x3List {
public:

  int num_matrices;
  int max_matrices;
  icMatrix3x3 **matrices;

  icMatrix3x3List (int max) {
    max_matrices = max;
    matrices = new icMatrix3x3 *[max_matrices];
    num_matrices = 0;
  }
	void append(icMatrix3x3 *m)
	{
		int i;

		if (num_matrices >= max_matrices) {
			max_matrices += 200;
			icMatrix3x3 **mlist = new icMatrix3x3 *[max_matrices];
			for (i = 0; i < num_matrices; i++)
				mlist[i] = matrices[i];
			delete (matrices);
			matrices = mlist;
